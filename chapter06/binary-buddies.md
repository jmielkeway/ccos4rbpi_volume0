...
Having setup the freelists to manage all of the available pages in the system, our OS needs to provide a sane interface through which clients can request access to blocks of memory. Extending the [src/allocate.c](code1/src/allocate.c) module to serve this purpose, we create the `alloc_pages` function, which takes in the order of the request as an argument:
...
The `freelists` are protected from concurrent access by the `allocator_lock`. After the client acquires the lock, each freelist, beginning with the smallest that could satisfy the request, is checked to see if any free blocks exist. If the list is not empty, the first page in the freelist is taken. Afterwards, the lock is released, allowing otheer clients a chance to continue.
...
If a block of memory was found, the freelists are again protected from concurrent access with the `allocator_lock`. The block found may have been of a higher order than what was requested. If the block found was of the proper size, the lock is immediately released. In the case the block found is of a higher order, to avoid wasting memory, the block is split into two halves. The first half, the buddy, is added to the freelist of one order less, and this information is encoded in the `current_order` field of the `struct page` representing the buddy. The second half is then updated to be a valid page, takes the same values for `current_order` and `original_order` members, and is used for the allocation. This splitting process then continues with the split block until the size of the block equals the size of the request.
...
+-----------------------------------------------------------------------------------------------------------------------------+
  ...
  ├------------------------------------------------------------------┤              +-----------------------------------------+
  | 0xFFFF000000808000 |  7:11 | ORIGINAL ORDER | 3                  |              | 0xFFFF000000206058 | 0xFFFF000000808108 |
  ├------------------------------------------------------------------┤              +-----------------------------------------+
  | 0xFFFF000000808000 | 12:63 | PFN            | 1024               |
  ├------------------------------------------------------------------┤
...
Given this setup, let's imagine the allocator receives a request to allocate a two-page block:
...
As both the freelist of order one, and the freelist of order two are empty, the first two iterations through the `for` loop result in an increment of `i`. On the third iteraton, we find a non-empty list with blocks of memory available for allocation. The first entry in the freelist of order three is a pointer to a list contained within a `struct page` structure located at address 0xFFFF000000808000. After the entry is found and removed from the freelist, our variables look like:
...
+-----------------------------------------------------------------------------------------------------------------------------+
  ...
  | 0xFFFF000000808000 |  3:6  | CURRENT ORDER  | 3    |                                       ▼                    ▼
  ├----------------------------------------------------┤                            +-----------------------------------------+
  | 0xFFFF000000808000 |  7:11 | ORIGINAL ORDER | 3    |                            | 0xFFFF000000206058 | 0xFFFF000000808208 |
  ├----------------------------------------------------┤                            +-----------------------------------------+
  | 0xFFFF000000808000 | 12:63 | PFN            | 1024 |
  ├----------------------------------------------------┤
...
- Grabs the page-frame-numer of the next order two page frame (half the size of our found page or order three), or the PFN four greater
...
+-----------------------------------------------------------------------------------------------------------------------------+
          ...
          ▼                    ▼                    ▼                    ▼                    ▼                    ▼
┌-----------------------------------------------------------------------------------------------------------------------------+
| 0xFFFF000000206038 | 0xFFFF000000206038 | 0xFFFF000000808008 | 0xFFFF000000808008 | 0xFFFF00000080BF08 | 0xFFFF000000808108 |
+-----------------------------------------------------------------------------------------------------------------------------+
  ...
  ├----------------------------------------------------┤
  | 0xFFFF000000808080 |  7:11 | ORIGINAL ORDER | 3    |
  ├----------------------------------------------------┤
  | 0xFFFF000000808080 | 12:63 | PFN            | 1028 |
  ├----------------------------------------------------┤
...
The original page we found to allocate has been broken into two pieces. The top half has been added back to the allocator, but as a page of order two. We still have a reference to the second half. Still, our allocation is too big, as the client requested an allocation of order one. So the loop repeats and:
...
- Grabs the page-frame-number of our found page
- Grabs the page-frame-numer of the next order one page frame (one-fourth the size of our found page of order three), or the PFN two greater
...
> Note in this case the current order of the page reference is only two, not three as the original allocation. As we will see in the example of the `free_pages` function, this is acceptable.
...
+-----------------------------------------------------------------------------------------------------------------------------+
          ...
          ▼                    ▼                    ▼                    ▼                    ▼                    ▼
┌-----------------------------------------------------------------------------------------------------------------------------+
| 0xFFFF000000808088 | 0xFFFF000000808088 | 0xFFFF000000808008 | 0xFFFF000000808008 | 0xFFFF00000080BF08 | 0xFFFF000000808108 |
+-----------------------------------------------------------------------------------------------------------------------------+
  ...
  ├----------------------------------------------------┤
  | 0xFFFF0000008080C0 |  7:11 | ORIGINAL ORDER | 2    |
  ├----------------------------------------------------┤
  | 0xFFFF0000008080C0 | 12:63 | PFN            | 1030 |
  ├----------------------------------------------------┤
...
After marking each of the pages in the freed block as unallocated, the `free_function` acquires the `allocate_lock` and determines if the freed page is the first or second half of the block one order larger. Then, able to locate the buddy of that block, it determines if coalescing is appropriate. If it is, it performs the coalescing operation. If it any point, coalescing should not be performed, the loop terminates, the page is added to the freelist of the page's current order, the lock is released, and the function exists.

As an example, let's revist the allocation example, and assume the same state only now freeing the page previously allocated. We pick up at the point of entering the `while` loop (so the page being free is no longer marked as allocated):
...
In order to determine if the page is the top half of the buddy, or the bottom half, the `free_pages` function uses the fact that the page frame is properly allocated to the order size. By applying a mask of the order one larger than the current to the page frame, the correct alignment is revealed. To determine if the page is a head buddy in the current state:

- (!((page->pfn) & ((1 << ((page->current_order) + 1)) - 1)))
- (!((1030) & ((1 << (1 + 1)) - 1)))
- !(1030 & (4 - 1))
...
- head buddy = &(GLOBAL_MEMMAP[HEAD_BUDDY_PFN(page)]))
- head buddy = &(GLOBAL_MEMMAP[((page->pfn) - (1 << (page->current_order)))]
- head buddy = &(GLOBAL_MEMMAP[1030 - (1 << 1)]
- head buddy = &(GLOBAL_MEMMAP[1030 - 2]
- head buddy = &(GLOBAL_MEMMAP[1028]

Now, having a reference to the head buddy, the `struct page` at index 1028 in the `GLOBAL_MEMMAP`, the buddy is removed from its freelist. The original page we are freeing is a tail page, and is combined with the buddy page, with the page reference being replaced by the buddy reference. The current order of the buddy increases by one. The original page disappears. Our visualization now looks like:
...
This is the same intermediate picture we saw during the allocation phase. It should be clear that the loop can continue one more time, and the state of the allocator will, and the termination of the loop, appear the same as when we started.

> Note: Here it should be clear that the tail buddy no longer is a valid page. This is why it was ok for the tail buddy to have a lower original order. It ceases to exist when coalesced.

The binary-buddy allocator is complete. Building and running will log copious output, walking thorugh allocates and frees to test our assumptions:

![Raspberry Pi Buddy Cheesecake](images/0601_rpi4_buddy.png)
