...
Creating more user space processes from our existing user startup process requires the `clone` and `exec` system calls to be available. Just as in the initialization of CheesecakeOS, clone is used to dynamically allocate a child process. The child will enter existence at `__ret_from_fork`, branch to `schedule_tail` for coherency, and then jump to the same location where the parent is continuing execution. At this point, execution can continue in the parent process. Or, the child can issue an `exec` on itself to provide itself with an entirely new execution context, and run a separate program.
...
The `exec` wrapper sets the `init` parameter to false, as discussed in the introduction to the exec function in [Chapter Nine](../chapter09/exec.md).

#### C Library Utilities

If our objective is to create a shell-like program that can take input from a user and generate further actions, we need some basic parsing functionality. At the very least we will need to compare input strings to statically allocated keys. In [arch/arm64/user/libc.c](code2/arch/arm64/user/libc.c), the `libc_strcmp` function will help us do just that:
...
The `libc_strlen` is another nice utility, especially for locating where to insert or remove a newline character from the end of a string.
...
The `Cheesecake Shell` proudly welcomes the user at the top of the loop.
...
If read returns with a string of some length, the user entered a full line, and the newline character is replaced with the null terminator to facilitate comparison. If the user entered an empty newline, by only pressing the enter key, control simply returns to the top of the loop, as the newline will already have been echoed.
...
We see the steak and potatoes of the shell function if the user enters "cat\n" at the command line. The `pid` variable is assigned to the return value of the `clone` system call, called with empty flags. Remember, clone returns twice, once in the parent, once in the child. The return value in the parent on success is greater than zero, or, less than zero on failure. The return value is zero in the child. If the return value is greater than zero, the parent has done its work, and control continues to the top of the inner loop where the shell emits the prompt again. An error message prints if clone returns a value less than zero. In the case clone returns zero, we know the child process is running. From the child process, an exec call is made, giving `cat` as the user space function to set as the program counter. The call to `exec` does not return, but branches to the `cat` function after creating a new address space and execution context for the process.
...
If the input string does not match any of the previous criteria, the shell complains of a program it cannot run, and the user is notified.
...
This to keep the compiler happy, as the shell application itself will never return.

Compiling and running our CheesecakeOS on your Raspberry Pi 4 will enable you to type commands into the first iteration of Cheesecake Shell. If you run the `cat` program, you may notice there is a problem! Both the shell and `cat` are competing for use of the TTY! When the shell issues a `read` call, it sleeps on the TTY waitqueue. The same is true for `cat`. So they are both sleeping on the waitqueue and one wakes up when the user presses enter. So they take turns back and forth, swapping ownership of the TTY buffer.

![Raspberry Pi Embryonic Cheesecake](images/1003_rpi4_embryonic.png)

This is not the expected behavior. Instead, we would expect to run the `cat` program in the _foreground_, while the shell relinquishes the TTY in the _background_. Our OS does not allow for this yet, because we have not implemented any type of job control. Luckily, this is the subject of the [next chapter!](../chapter11/chapter11.md). See you there!
