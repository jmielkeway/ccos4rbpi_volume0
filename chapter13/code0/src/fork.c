...
extern int copy_arch_context(unsigned long flags,
    ...
    unsigned long arg,
    struct process *p);
extern void deallocate_pid(int pid);
...
static int copy_memmap(unsigned long flags, struct process *p)
{
    ...
    unsigned long arg)
{
    ...
    if(copy_memmap(flags, p)) {
        goto freeprocess;
    }
    if(copy_signal(flags, p)) {
        goto freememmap;
    }
...
static long copy_signal(unsigned long flags, struct process *p)
{
    if(flags & CLONE_SIGNAL) {
        ATOMIC_LONG_INC(&(p->signal->refcount));
        return 0;
    }
